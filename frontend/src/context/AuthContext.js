...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    const savedToken = localStorage.getItem('smartstay_token');
  ...
  const login = (userData, userToken) => {
    setUser(userData);
    setToken(userToken);
    localStorage.setItem('smartstay_token', userToken);
    localStorage.setItem('smartstay_user', JSON.stringify(userData));
  };
